...
int arrayAlumno_remove(Alumno* array[],int cantidadMaxima,int* cantidadActual, int id)
{
    int retorno = -1;
    int i;
    int auxId;

    if(array != NULL && *cantidadActual != NULL && id > 0)
    {   retorno = -2;
        for(i=0;i<cantidadActual;i++)
        {
            alumno_getId(array[i], &auxId);
            if(auxId == id)
            {
                retorno =0;
                alumno_delete(array[i]);
            }
        }
    }
